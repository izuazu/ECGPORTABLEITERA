document.addEventListener('DOMContentLoaded', () => {
    ...
    function initCharts() {
        ...
        ecgChart = new Chart(ecgCtx, {
            ...
            data: { labels: [], datasets: [{
                label: 'Sinyal EKG', data: [], borderColor: 'rgb(75, 192, 192)',
            ...
            options: {
                ...
                scales: {
                    x: { display: false },
                    y: { min: -2, max: 2, beginAtZero: false, ticks: { stepSize: 0.5 } }
                }
            }
        });
        ...
        bpmChart = new Chart(bpmCtx, {
            type: 'line',
            data: { labels: [], datasets: [{
                label: 'BPM', data: [], borderColor: 'rgb(255, 99, 132)',
                backgroundColor: 'rgba(255, 99, 132, 0.2)', borderWidth: 2, fill: true
            }] },
    ...
    function connectMqtt() {
        ...
        client.connect({
            ...
            userName: MQTT_USERNAME, password: MQTT_PASSWORD, useSSL: true
        });
    }
    ...
    function updateDashboard(logEntry) {
        ...
        if (topic === ECG_TOPIC && !isNaN(value)) {
            ...
            updateChartData(bpmChart, time, Math.round(value));
    ...
    function setupEventListeners() {
        ...
        document.getElementById('downloadEcgChart').addEventListener('click', () => {
            downloadFile('grafik_ekg.png', ecgChart.toBase64Image(), 'image/png');
        });

        document.getElementById('downloadBpmChart').addEventListener('click', () => {
            downloadFile('grafik_bpm.png', bpmChart.toBase64Image(), 'image/png');
        });

        document.getElementById('downloadXlsx').addEventListener('click', () => {
            if (dataLog.length === 0) return alert('Tidak ada data untuk diunduh.');
            const worksheet = XLSX.utils.json_to_sheet(dataLog);
            const workbook = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(workbook, worksheet, "Data EKG");
            XLSX.writeFile(workbook, "ekg_data.xlsx");
        });

        document.getElementById('downloadCsv').addEventListener('click', () => {
            if (dataLog.length === 0) return alert('Tidak ada data untuk diunduh.');
            let csvContent = 'timestamp,topic,payload\n';
            dataLog.forEach(l => { csvContent += `"${l.timestamp}","${l.topic}","${l.payload}"\n`; });
            downloadFile('ekg_data.csv', csvContent, 'text/csv;charset=utf-8;');
        });

        document.getElementById('downloadTxt').addEventListener('click', () => {
            if (dataLog.length === 0) return alert('Tidak ada data untuk diunduh.');
            let txtContent = 'Log Data EKG\n====================\n';
            dataLog.forEach(l => { txtContent += `[${l.timestamp}] | Topic: ${l.topic} | Payload: ${l.payload}\n`; });
            downloadFile('ekg_data.txt', txtContent, 'text/plain;charset=utf-8;');
        });

        document.getElementById('resetData').addEventListener('click', () => {
            if (confirm('Anda yakin ingin mereset semua data pada dashboard?')) {
                resetAllData();
            }
        });
    }
